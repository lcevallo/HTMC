...
public class StockSFItemBean implements Serializable {
	...
	public void setDtblStockRealPivote(List<ConsolidadoStockRealPivoteSf> dtblStockRealPivote) {
	...
	public void setFiltradoStockRealPivote(List<ConsolidadoStockRealPivoteSf> filtradoStockRealPivote) {
	...
	/**
	 * @param selectedColumnsStocks
	 *            the selectedColumnsStocks to set
	 */
	...
	public boolean filtrarConRango(Object value, Object filter, Locale locale) {
		...
		if (filterText.matches(pattern))
			return new BigDecimal(value.toString()).compareTo(new BigDecimal(filterText)) == 0;
		...
		if (filterText.matches(pattern)) {
			...
			return ((Comparable) new BigDecimal(value.toString())).compareTo(new BigDecimal(texto)) < 0;
		}
	...
	public void verStockAlCortePdf() {
		...
		if (this.dtblStockRealPivote != null) {
			...
			try {
				...
				RequestContext.getCurrentInstance().update("impresionReporteDialogWidget");
	...
	public void setDialogRendered(boolean dialogHidden) {
		this.dialogRendered = dialogHidden;
	}
