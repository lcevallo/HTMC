		...
		if (StringMD5.getStringMessageDigest(actualPassword, "MD5").equals(userInSession.getClave())) {
			...
			try {
				...
				FacesContext.getCurrentInstance().addMessage(null,
						new FacesMessage(FacesMessage.SEVERITY_INFO, "Contraseña cambiada correctamente", null));
			} catch (Exception exception) {
				System.out.println(exception.getMessage());
			}
		} else {
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_ERROR, "La contraseña actual no es correcta", null));
